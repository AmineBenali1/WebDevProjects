<template>
  <div class="job-form">
    <h2>{{ isEditing ? 'Modifier l\'emploi' : 'Ajouter un nouvel emploi' }}</h2>
    <form @submit.prevent="handleSubmit">
      ...
      <div class="form-group">
        ...
        <input type="text" v-model="form.salaire" required>
      </div>
      <div class="form-group">
        <label for="dateCreation">Date de création</label>
        <input type="date" v-model="form.dateCreation" required>
      </div>
      <div class="form-group">
...
export default {
  ...
  created() {
    if (this.isEditing) {
      this.form = {
        titre: this.editingJob.titre,
        description: this.editingJob.description,
        salaire: this.editingJob.salaire,
        dateCreation: this.editingJob["date de création"],
        experience: this.editingJob["années d'expérience"] || ''
      }
    }
  },
  methods: {
